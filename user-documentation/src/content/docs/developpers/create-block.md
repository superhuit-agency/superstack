...
subcategory: 'Block'
summary: 'Les deux façons de créer un contenu bloc dans la stack : pattern ou bloc Gutenberg personnalisé.'
order: 1
---
...
- Paste it into a new file under `wordpress/theme/patterns/`.
...
- Add matching styles under `next/src/css/patterns/`.
- Optionally add editor preview styles under `wordpress/theme/src/shared/`.
...
Add the slug to `blocksList` in `next/src/components/global/Blocks.tsx`:
...
Create files under `wordpress/theme/src/blocks/<name>/`:
...
**`edit.tsx`** — build the Gutenberg UI with `@wordpress/block-editor` components (`RichText`, `useBlockProps`, `InnerBlocks`, etc.). See `wordpress/theme/src/blocks/benefits-list-item/edit.tsx`.

### Step 4: Wire the block into the editor bundle

Add an import in `wordpress/theme/src/blocks/index.ts`:

```ts
import './benefits-list-item/register';
```

Ensure `wordpress/theme/src/editor/editor.ts` loads the barrel:
...
To tweak an existing core block (e.g. limit heading levels, restrict post types), add an `edit.tsx` filter under `next/src/components/core/<Block>/` and export it from `next/src/components/filters.ts`. This is different from registering a new custom block — see `next/src/components/core/Heading/README.md`.
