...
async function getSitemapTypeUrls(type = 'posts', page = 1, size = 1000) {
  ...
  if (size > GRAPHQL_MAX_SIZE) {
    (
      await Promise.allSettled(
        new Array(Math.ceil(size / GRAPHQL_MAX_SIZE)).fill(0).map((v, i) =>
          fetchAPI(
            `query SitemapTypeUrls{
                      ...
                      (page - 1) * size + i * GRAPHQL_MAX_SIZE
                    }}
									}
								) {
									edges {
  ...
  return nodes.reduce((urls, { node }) => {
    if (node.seo.metaRobotsNoindex === 'index')
      urls.push(parseNodeTranslations(parseNodeImages(parseNodeDate(node))));
    return urls;
  }, []);
}
