...
const pickAdjacentNode = (
  ...
  normalizeCandidates(nodes).find((node) => node.databaseId !== currentDatabaseId) ??
  null;

/** WPGraphQL `dateQuery.before` / `after` expect `DateInput`, not a plain string. */
const toWpDateInput = (isoDate: string): { year: number; month: number; day: number } => {
...
export const getData = async (
  ...
  const categoryIn =
    taxonomy === 'category'
      ? currentNode?.categories?.nodes
          ?.map((term: { databaseId: number }) => Number(term.databaseId))
  ...
  const fallbackPagesQuery = gql`
    query PostNavigationLinkFallbackPages($notIn: [ID]) {
      pages(first: 20, where: { orderby: { field: DATE, order: ASC }, notIn: $notIn }) {
        nodes {
          databaseId
          date
          uri
  ...
  try {
    if (currentNode.__typename === 'Post') {
      ...
      adjacentPost = pickAdjacentNode(postData?.posts?.nodes, currentNode.databaseId);
    } else {
      const pageData = await fetcher(pageAdjacentQuery, {
        variables: commonVariables,
      });
      adjacentPost = pickAdjacentNode(pageData?.pages?.nodes, currentNode.databaseId);
    }
  } catch {
    if (currentNode.__typename === 'Post') {
