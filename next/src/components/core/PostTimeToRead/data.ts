    ...
    for (const block of list) {
      ...
      if (Array.isArray(block.innerContent)) {
        for (const chunk of block.innerContent) {
          if (typeof chunk === 'string' && chunk.trim()) parts.push(chunk);
        }
      }
...
const pickTextForWordCount = (
  ...
  const rendered =
    typeof bundle?.renderedContent === 'string' ? bundle.renderedContent : '';

  if (fromBlocks.trim().length > 0 && rendered.length >= RENDERED_FALLBACK_MIN_LEN) {
    const wcBlocks = blockCorePostTimeToReadWordCount(fromBlocks, wordCountType);
...
export const getData = async (
  ...
  const averageReadingSpeed = normalizeReadingSpeed(attrs?.averageReadingSpeed);
  ...
  const totalUnits = blockCorePostTimeToReadWordCount(textSource, wordCountType);
  ...
  return {
    ...base,
    readingMinutes: Math.max(1, Math.round(totalUnits / averageReadingSpeed)),
  };
};
