...
const mergeTermQuery = (
  ...
  ...DEFAULT_TERM_QUERY,
  ...partial,
  include: Array.isArray(partial?.include) ? partial!.include : DEFAULT_TERM_QUERY.include,
});
...
export const getData = async (
  ...
  const first = Math.min(100, Math.max(1, toPositiveInt(termQuery.perPage) ?? 10));
