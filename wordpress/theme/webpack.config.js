		...
		compiler.hooks.afterCompile.tap('CleanCSSErrors', (compilation) => {
			for (const error of compilation.errors) {
				...
				for (const line of lines) {
					if (line.includes('___CSS_LOADER') || line.includes('sourceMappingURL') || line.startsWith('","sourceRoot"')) break;
					kept.push(line);
				}
...
module.exports = {
	...
	output: {
		...
		filename: (pathData) => {
			...
			const originalFilename = defaultConfig.output.filename || '[name].js';
			let filename;
		...
		chunkFilename: (pathData) => {
			...
			const originalChunkFilename = defaultConfig.output.chunkFilename || '[id].js';
			let filename;
			if (typeof originalChunkFilename === 'function') {
				filename = originalChunkFilename(pathData);
			} else {
				const chunkId = pathData.chunk.id || pathData.chunk.name || 'main';
				filename = originalChunkFilename.replace('[id]', chunkId).replace('[name]', pathData.chunk.name || chunkId);
			}
			// Replace .ts.js with .js
			return filename.replace(/\.ts\.js$/, '.js');
		},
	...
	plugins: [
		...
		...defaultConfig.plugins.map((plugin) => {
			...
			if (plugin instanceof MiniCssExtractPlugin) {
				const originalFilename = plugin.options.filename || '[name].css';
				const originalChunkFilename = plugin.options.chunkFilename || '[id].css';
