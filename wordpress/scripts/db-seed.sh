...
# next_url arrives pointing at the source instance's Next.js, and the search
# above does not touch it — a bare "http://localhost" source URL is a prefix of
# "http://localhost:3000", so replacing it would mangle the port. Set it
# outright instead, the same way provision.sh does on boot.
if [ -n "$NEXT_URL" ]; then
	docker exec "${PROJECT_CODE}_wp" wp option update next_url "$NEXT_URL" --quiet || true
fi
